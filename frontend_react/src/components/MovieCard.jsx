...
function MovieCard({ movie }) {
  ...
  const { favourites, addFavourite, deleteFavourite } =
    useContext(FavouritesContext);
  const { watchLater, addWatchLater, deleteWatchLater } =
  ...
  return (
    <div className="bg-[#0d1117] rounded-lg p-2 relative">
      <div className="relative">
        ...
        >
          <p className="text-3xl">{isFavourite ? "❤️" : "🤍"}</p>
      ...
      <div className="text-white text-lg font-normal mt-2">
        ...
        <div className="mt-2 text-sm font-normal flex gap-2 items-center">
          ...
          <div className="ml-auto mr-3 relative group">
            <button
              className="text-white text-xl font-extrabold cursor-pointer p-2"
              onClick={() =>
                setMovieOptionId(movieOptionId === tmdb_id ? null : tmdb_id)
              }
            >
              ⋮
            </button>
